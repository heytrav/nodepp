        ...
        "initStream": function(onConnectCallback) {
            ...
            try {
                ...
                newStream.on('readable', function() {
                    var readChunk = newStream.read();
                    console.log("Readable received: ",readChunk.toString());
                    var bigEndian = readChunk.slice(0,3);
                    var bigEndianLength = bigEndian.readUInt32BE(0) - 4;
                    var bodyChunk = readChunk.slice(4);
                    console.log("Received data: ", bodyChunk.toString());
                    self.clientResponse(bodyChunk);
