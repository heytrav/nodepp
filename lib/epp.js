...
module.exports = function(registry, config) {
    return {
        ...
        "processAuthCode": function(data, namespace) {
            ...
            if (authType === "string" || authType === "number") {
                ...
                if (!data.pw) throw 'pw is required!'; 
                authCode = data.pw;
                if (data.roid) authCodeData[pwKey]._attr = {
                    "roid": data.roid
                };
            }
            authCodeData[pwKey]._value = authCode;
            return authCodeData;
        },
        ...
        "createDomain": function(data, transactionId) {
            ...
            createData["domain:period"] = this.processDomainPeriod(data.period);
            if (data.authInfo) {
