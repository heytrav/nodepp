...
EPP.prototype.processPostalAddressItem = function(curAddr) {
    this.processAlternativeSyntax('cc', curAddr, ["country", "ccode"]);
    this.processAlternativeSyntax('pc', curAddr, ["pcode", "postcode", "zip"]);
    this.processAlternativeSyntax('sp', curAddr, ["state"]);
    var preppedCurAddr = {
...
EPP.prototype.processPostalInfoItem = function(postalInfo) {
    this.processAlternativeSyntax('org', postalInfo, ["company", "organization", "organisation"]);
...
/*
 * Convert various alternative attributes into what EPP wants.
 * */
EPP.prototype.processAlternativeSyntax = function(key, data, alternatives) {
...
EPP.prototype.processContactData = function(data) {
    this.processAlternativeSyntax('voice', data, ["tel", "telephone", "phone"]);
...
EPP.prototype.transferContact = function(data, transactionId) {
    ...
    if (data.authInfo) transferData["contact:authInfo"] = this.processAuthInfo(data.authInfo, 'contact');
    else throw new Error("An 'authInfo' is required!");
    var xml = this.eppCommand({
        "transfer": {
            "_attr": {
