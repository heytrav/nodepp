...
EPP.prototype.processContactData = function(data) {
    ...
    if (data.authInfo) processedContactData['contact:authInfo'] = this.processAuthInfo(data.authInfo, 'contact');
    if (data.disclose) {
...
EPP.prototype.infoDomain = function(data, transactionId) {
    ...
    this.normaliseVariants('authInfo', data, ["authcode", "authCode", "auth_code", "password", "pw"]);
    if (data.authInfo) infoData['domain:authInfo'] = this.processAuthInfo(data.authInfo, 'domain');
    var xml = this.eppCommand({
        "info": infoData
    },
    transactionId);
...
EPP.prototype.createDomain = function(data, transactionId) {
    ...
    if (data.authInfo) createData['domain:authInfo'] = this.processAuthInfo(data.authInfo, 'domain');
    var commandData = {
        "create": {
...
EPP.prototype.updateDomain = function(data, transactionId) {
    ...
    if (change) {
        ...
        if (change.authInfo) changeData['domain:authInfo'] = this.processAuthInfo(change.authInfo, 'domain');
